  ...
  <div class="sidebar" :class="sidebarSelectedFilter.class">
    <vue-perfect-scrollbar class="scroll-area" :settings="settings">
      <div class="transparent navigation">
        <v-list>
          <app-logo></app-logo>
          <template v-if="usertype == 0" v-for="(category, key) in menus">
            <div :key="key">
          ...
          <template v-if="usertype !== 0" v-for="(category, key) in othermenus">
            <div :key="key">
              <div class="sidebar-title px-4">
                <span>{{ $t(key) }}</span>
              </div>
...
export default {
  data() {
    let type = JSON.parse(localStorage.getItem("user")).usertype;
    console.log(type);
    return {
      usertype: type,
      settings: {
  ...
  computed: {
    ...mapGetters(["sidebarSelectedFilter", "menus", "othermenus"])
  },
  methods: {
    textTruncate(text) {
